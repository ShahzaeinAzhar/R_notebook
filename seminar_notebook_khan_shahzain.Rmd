...
file_path <- if (Sys.info()["sysname"] == "Windows") {
  normalizePath("C:/users/Fujitsu/Desktop/R_notebook/fraud_detection.csv")
} else {
  normalizePath("~/Repos/R_Code/canada_online_scam/fraud_detection.csv")
}

data <- read.csv(file=file_path, check.names = FALSE, header = TRUE)
...
data <- read.csv(file = file_path, check.names = FALSE, header = TRUE, row.names = NULL)
...
# file_path_unwanted <- if (Sys.info()["sysname"] == "Windows") {
#   normalizePath("C:/users/Fujitsu/Desktop/R_notebook/unwanted_data.xlsx")
# } else {
#   normalizePath("~/Repos/R_Code/canada_online_scam/unwanted_data.xlsx")
# }

# write.xlsx(filtered_data$cleaned_data, file = file_path)  # Change to .xlsx for Excel compatibility
# write.xlsx(filtered_data$unwanted_data, file = file_path_unwanted) # Change to .xlsx for Excel compatibility
# 
# # Set Dollar_Loss_(CAD) column as text format
# write.xlsx(cleaned_data, file = file_path, sheet.name = "Cleaned_Data", colNames = TRUE, rowNames = TRUE) # Change to .xlsx for Excel compatibility

file_path <- if (Sys.info()["sysname"] == "Windows") {
  normalizePath("C:/users/Fujitsu/Desktop/R_notebook/cleaned_data.xlsx")
} else {
  normalizePath("~/Repos/R_Code/canada_online_scam/cleaned_data.xlsx")
}
...
chi_square_gender_complaint <- read.xlsx(file_path)
...
regression_gender_scam_types <- read.xlsx(file_path)
...
anova_test_fraud_type_and_age <- read.xlsx(file_path)
...
relation_age_gender_dollar_loss <- read.xlsx(file_path)
...
no_of_complaint_provinces <- read.xlsx(file_path)
...
specific_scam_in_provinces <- read.xlsx(file_path)
...
no_victims_trends_seasonal_cyclical <- read.xlsx(file_path)
...
```{r results='asis'}
data_clean_notebook <- read.xlsx(file_path)
```
...
data_unclean_notebook <- read.xlsx(file_path)
